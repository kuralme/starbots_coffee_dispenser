...
class ObjectDetection(Node):
    def __init__(self) -> None:
        super().__init__('object_detection_real_node')
    ...
    def callback(self, msg: PointCloud2) -> None:
        try:
            ...
            filtered_cloud_plane = self.filter_cloud(cloud, min_x_dist=-0.1, max_x_dist=0.4, min_y_dist=0.0, max_y_dist=0.75, min_height=-0.1, max_height=0.02)
            # Filtered cloud for object detection
            filtered_cloud_objects = self.filter_cloud(cloud, min_x_dist=-0.1, max_x_dist=0.4, min_y_dist=0.2, max_y_dist=0.7, min_height=0.0, max_height=0.2)
    ...
    def filter_cloud(self, cloud: pcl.PointCloud, min_x_dist: float, max_x_dist: float, min_y_dist: float, max_y_dist: float, min_height: float, max_height: float) -> Union[pcl.PointCloud, None]:
        # Filter cloud
        try:
            indices = []
            ...
            for i in range(cloud.size):
                point = cloud[i]
                if (min_x_dist <= point[0] <= max_x_dist) and (min_y_dist <= point[1] <= max_y_dist) and (min_height <= point[2] <= max_height):
                    indices.append(i)
    ...
    def extract_clusters(self, cloud: pcl.PointCloud, cluster_type: str) -> Tuple[List[pcl.PointCloud], List[List[float]], List[List[float]]]:
        """Extracts clusters corresponding to tables from the point cloud"""
        ...
        return table_clusters, cluster_centroids, cluster_dimensions
